...
import { Plus, FileText, Calendar, FileCheck } from 'lucide-react';
import Link from 'next/link';

export default async function MCPReportsPage() {
        ...
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-sm text-slate-500">Monthly Church Program reports and activities</p>
                </div>
                <Link href="/reports/mcp/create">
                    <Button>
            ...
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                {(!reports || reports.length === 0) ? (
                    <div className="col-span-full py-12 text-center border-2 border-dashed border-slate-200 rounded-xl bg-white/50">
                        ...
                        <p className="text-slate-500 font-medium">No MCP reports yet</p>
                    </div>
                ) : (
                    reports.map((report) => (
                        <Card key={report.id} className="hover:shadow-md transition-all border-slate-200">
                            ...
                            <CardContent className="space-y-4">
                                ...
                                <div className="flex items-center justify-between pt-2 border-t border-slate-100">
                                    ...
                                    <Link href={`/reports/mcp/${report.id}`} className="text-sm font-medium text-blue-600 hover:text-blue-700">View</Link>
                                </div>
                            </CardContent>
                        </Card>
                    ))
