...
export default async function GTVETReportsPage() {
        ...
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-sm text-slate-500">Ghana Technical and Vocational Education and Training reports</p>
                </div>
                <Link href="/reports/gtvet/create">
            ...
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                {(!reports || reports.length === 0) ? (
                    <div className="col-span-full py-12 text-center border-2 border-dashed border-slate-200 rounded-xl bg-white/50">
                        ...
                        <p className="text-slate-500 font-medium">No GTVET reports yet</p>
                    </div>
                ) : (
                    reports.map((report) => (
                        <Card key={report.id} className="hover:shadow-md transition-all border-slate-200 group">
                            ...
                            <CardContent className="space-y-4">
                                ...
                                <div className="flex items-center justify-between pt-2 border-t border-slate-100">
                                    ...
                                    <Link href={`/reports/gtvet/${report.id}`} className="text-sm font-bold text-purple-600 hover:text-purple-700">
                                        View Details
                                    </Link>
                                </div>
                            </CardContent>
