...
import { Plus, FileText, Calendar, School } from 'lucide-react';
import Link from 'next/link';

export default async function AdidomeVocationalReportsPage() {
    const supabase = await createClient();

    const { data: reports, error } = await supabase
        .from('adidome_vocational_reports')
        .select(`
      *,
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <h2 className="text-2xl font-bold text-slate-900 flex items-center gap-2">
                        ...
                        Adidome Vocational School Reports
                    </h2>
                    <p className="text-sm text-slate-500">Track activities and progress at Adidome Vocational School</p>
                </div>
                <Link href="/reports/adidome-vocational/create">
            ...
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                {(!reports || reports.length === 0) ? (
                    <div className="col-span-full py-12 text-center border-2 border-dashed border-slate-200 rounded-xl bg-white/50">
                        ...
                        <p className="text-slate-500 font-medium">No Adidome Vocational School reports yet</p>
                    </div>
                ) : (
                    reports.map((report) => (
                        <Card key={report.id} className="hover:shadow-md transition-all border-slate-200 group">
                            ...
                            <CardContent className="space-y-4">
                                ...
                                <div className="flex items-center justify-between pt-2 border-t border-slate-100">
                                    ...
                                    <Link href={`/reports/adidome-vocational/${report.id}`} className="text-sm font-bold text-orange-600 hover:text-orange-700">
                                        View Details
                                    </Link>
                                </div>
                            </CardContent>
